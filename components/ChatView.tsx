...
const ChatView: React.FC<Props> = ({ conversation, onBack, onSendMessage, onReceiveMessage, onNavigateToHelp }) => {
  ...
  // AI Response Logic with error handling
  useEffect(() => {
  ...
  const handleCall = () => {
    ...
    if (dialable) {
      ...
      // Auto-close after 3 seconds for demo effect
      setTimeout(() => {
        setActiveOverlay('none');
      }, 3000);
    }
  ...
  const toggleOverlay = (type: OverlayType) => {
    setActiveOverlay(activeOverlay === type ? 'none' : type);
  };

  // Grouping messages by date for "Past Messages" display
  const groupedMessages = useMemo(() => {
    const groups: { date: string, messages: Message[] }[] = [];
    
    // Messages are already sorted in state
  ...
  return (
    <div className="flex flex-col h-full bg-[#0b141b] text-white relative select-none overflow-hidden">
        ...
        <div className="flex items-center pr-2 relative">
          <button onClick={handleCall} className="p-3 text-[#e3e3e3] hover:bg-gray-800 rounded-full active:scale-90 transition-all">
            <svg className="w-[24px] h-[24px]" fill="currentColor" viewBox="0 0 24 24">
               <path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97c-2.83-1.35-5.43-3.9-6.63-6.66l1.97-1.57c.27-.27.35-.66.24-1.02-.36-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19c-.54 0-1 .45-1 .99 0 9.39 7.61 17 17 17 .55 0 1-.45 1-.99v-3.89c0-.54-.45-.99-.99-.99z"/>
            </svg>
          </button>
          <button 
            onClick={() => setIsMoreMenuOpen(!isMoreMenuOpen)}
            className={`p-3 text-[#e3e3e3] rounded-full transition-colors ${isMoreMenuOpen ? 'bg-gray-800' : 'hover:bg-gray-800 active:bg-gray-800'}`}
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {isMoreMenuOpen && (
            <div ref={menuRef} className="absolute top-1 right-2 w-[240px] bg-[#1f2933] rounded-[12px] shadow-2xl z-50 py-3 animate-fade-in border border-gray-700/30">
              ...
              <MenuItem label="Delete conversation" onClick={() => {
                if(confirm("Delete this entire conversation history?")) {
                  onBack();
                }
                setIsMoreMenuOpen(false);
              }} />
      ...
      {activeOverlay === 'calling' && (
        <div className="absolute inset-0 bg-[#0b141b] z-[100] flex flex-col items-center justify-center animate-fade-in">
          <div className="flex flex-col items-center space-y-8">
            <div className="w-32 h-32 rounded-full bg-[#fbc02d] flex items-center justify-center text-4xl font-bold text-[#0b141b] shadow-2xl animate-pulse">
        ...
        {groupedMessages.map((group, gIdx) => (
          <React.Fragment key={group.date}>
            <div className="flex justify-center my-6">
               <span className="text-[12px] text-gray-400 font-medium tracking-wide bg-[#1f2933]/60 px-4 py-1.5 rounded-full uppercase border border-gray-800/50">
                 {group.date}
               </span>
            </div>
            {group.messages.map((msg, mIdx) => (
              <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'} mb-1`}>
                <div className={`max-w-[85%] px-4 py-3 rounded-[24px] text-[16px] font-roboto leading-[1.4] shadow-sm transition-all duration-200 ${
                  msg.sender === 'me' 
                  ? 'bg-[#1b72e8] text-white rounded-br-[4px]' 
                  : 'bg-[#1f2933] text-[#e3e3e3] rounded-bl-[4px]'
                }`}>
                  {msg.image && (
                    <img 
                      src={msg.image} 
                      alt="Attachment" 
                      className="max-w-full rounded-[16px] mb-2 border border-white/10" 
                      referrerPolicy="no-referrer"
                    />
                  )}
                  {msg.audio && (
                    <div className="mb-2">
                      <audio controls src={msg.audio} className="w-full h-8 filter invert opacity-80" />
                    </div>
                  )}
                  {msg.text}
      ...
      {showScrollToTop && (
        <button
          onClick={scrollToTop}
          className="absolute bottom-24 right-4 w-12 h-12 bg-[#1f2933] rounded-full flex items-center justify-center shadow-lg active:scale-90 transition-all hover:bg-[#2d3748] z-10"
      ...
      <div className="relative bg-[#0b141b]">
          ...
          <div className="absolute bottom-full left-0 w-full h-[250px] bg-[#1f2933] border-t border-gray-700/50 overflow-y-auto p-4 grid grid-cols-8 gap-2 animate-slide-up z-30">
            {EMOJIS.map((emoji, index) => (
              <button 
                ...
                className="text-2xl hover:bg-gray-700 p-2 rounded-lg transition-colors active:scale-90"
              >
                {emoji}
              </button>
            ))}
        ...
        />
        <div className="px-4 pb-10 pt-2 flex items-center space-x-3 safe-area-bottom bg-[#0b141b] w-full">
          <button onClick={handleFileClick} className={`w-12 h-12 flex items-center justify-center rounded-full transition-all active:scale-90 flex-shrink-0 hover:bg-[#2d3748] ${activeOverlay === 'attachments' ? 'bg-[#7fcfff] text-[#0b141b]' : 'bg-[#1f2933] text-[#7fcfff]'}`}>
          ...
          <div className="flex-1 flex items-center bg-[#1f2933] rounded-[28px] px-5 py-2.5 overflow-hidden transition-colors hover:bg-[#2d3748]">
            <input 
              ...
              className="flex-1 bg-transparent border-none focus:ring-0 text-[16px] text-white outline-none placeholder-gray-400 min-w-0 font-roboto"
              value={inputText}
              onChange={(e) => setInputText(e.target.value)}
              onKeyPress={(e) => e.key === 'Enter' && handleSend()}
              onFocus={() => setActiveOverlay('none')}
            />
            <div className="flex items-center space-x-4 ml-3 flex-shrink-0">
              <button onClick={() => toggleOverlay('emoji')} className="active:scale-90 text-gray-400 hover:text-white transition-colors">
          ...
          {isInputEmpty ? (
            <div className="flex items-center space-x-2">
              ...
              <button 
                ...
                className={`w-12 h-12 flex items-center justify-center rounded-full transition-all active:scale-90 flex-shrink-0 ${isRecording ? 'bg-red-500 text-white animate-pulse' : 'bg-[#1f2933] text-[#7fcfff] hover:bg-[#2d3748]'}`}
              >
                {isRecording ? (
            ...
            <button onClick={() => handleSend()} className="w-12 h-12 flex flex-col items-center justify-center rounded-full bg-[#a8c7fa] flex-shrink-0 active:scale-90 transition-all shadow-md">
              <div className="relative flex flex-col items-center pt-1">
                  <span className="text-[9px] font-bold text-[#062e6f] mt-[-2px]">SMS</span>
                </div>
            </button>
          )}
        </div>
