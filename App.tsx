...
const App: React.FC = () => {
    ...
    try {
      ...
      for (const phoneKey of Object.keys(grouped)) {
        ...
        if (conversationExists) {
          // Check 1-second window for existing conversations
          const lastReadTime = existing?.lastReadTime || 0;
          const timeSinceLastRead = Date.now() - lastReadTime;
          const shouldIncrement = timeSinceLastRead > 1000; // 1 second
  ...
  const handleCbeMessage = async (cbeMessage: any) => {
    ...
    // Only increment if user hasn't opened conversation recently (1-second window)
    const timeSinceLastRead = Date.now() - lastReadTime;
    const shouldIncrement = timeSinceLastRead > 1000; // 1 second
